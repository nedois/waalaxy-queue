...
describe('InMemoryQueue', () => {
  ...
  describe('enqueue', () => {
    it("should add a valid action to the user's queue", async () => {
      ...
      const queue = await inMemoryQueue.peek(action.userId);
    ...
    it('should append and action to the existing queue for the user', async () => {
      ...
      let queue = await inMemoryQueue.peek(action1.userId);
      expect(queue).toHaveLength(1);
      expect(queue).toContain(action1);

      inMemoryQueue.enqueue(action2);
      queue = await inMemoryQueue.peek(action1.userId);
  ...
  describe('remove', () => {
    it('should remove an action from the user queue', async () => {
      ...
      let queue = await inMemoryQueue.peek(action.userId);
      expect(queue).toContain(action);

      action.status = 'COMPLETED';
      actionRepository.save(action);
      inMemoryQueue.remove(action);
      queue = await inMemoryQueue.peek(action.userId);
  ...
  describe('peek', () => {
    it('should return an empty array if the user has no actions in the queue', async () => {
      const actions = await inMemoryQueue.peek('4310a855-19b2-4af7-815e-019cfa5c31d1');
    ...
    it('should return the actions in the user queue', async () => {
      ...
      const actions = await inMemoryQueue.peek(action1.userId);
